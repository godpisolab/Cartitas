...
Cada lanzamiento tipo booster se siembra hasta tres veces (sobre + caja + case) porque las tiendas los venden como SKUs distintos con precios muy distintos -- las tres viven en la MISMA categoría de família (`one-piece`/`extra-booster`/`premium-booster-box`), distinguidas por `product.packaging`, no en categorías separadas (ver más abajo). Los productos sin categoría en la taxonomía de 10 tipos matcheables (fundas sin línea reconocida, binders, cajas de almacenaje sueltas, sets de aniversario...) se omiten y se listan al final de la ejecución.

**Recognition Pipeline (2026-08-30, `docs/propuestas/guia_nuevo_matcher.md`):** `classify_product()`/`matcher.py` se rediseñaron por completo. Cambios principales respecto a lo que describían las entradas anteriores de esta sección (ya no vigentes, se dejan las siguientes solo como historial):

- `BOOSTER_BOX`/`BOOSTER_PACK`/`BOOSTER_CASE` se funden en un único tipo `ONE_PIECE` -- la distinción sobre/caja/case pasa a ser `Classification.packaging` (`"sobre"`/`"display"`/`"case"`), calculado dentro del pipeline en vez de por categoría o por `is_box_variant()`/`_BOOSTER_CASE_RE`. Mismo tratamiento para `EXTRA_BOOSTER` y `PREMIUM_BOOSTER_BOX` (família nueva, antes parte de `PREMIUM_COLLECTION`).
- `PREMIUM_COLLECTION` se separa en `PREMIUM_CARD_COLLECTION` y `PREMIUM_BOOSTER_BOX` -- son productos distintos (no una variante de empaquetado del mismo), cada uno con su propia categoría.
- `LEARN_DECK` se funde en `STARTER_DECK`. `SLEEVES` es família nueva (antes caía en `OTROS`).
- `PROMO_CARD`/`MYSTERY_PACK`/`DICE_ACCESSORY` suben a Fase 0 del pipeline (`not_applicable` siempre, igual que `LOTE_CARTAS`) -- ya no tienen categoría propia ni entran nunca en `_best_candidate()`; decisión de diseño explícita, no un hueco de cobertura (ninguno tiene un canónico de producto sellado razonable con el que comparar precio).
- Categorías-hoja: 10 (`one-piece`, `extra-booster`, `premium-booster-box`, `premium-card-collection`, `starter-deck`, `illustration-box`, `double-pack`, `devil-fruits-collection`, `sleeves`, `playmat`), antes 13.

Umbrales (aprobados 2026-08-26 como valores de partida, calibrados con datos reales el 2026-08-27, y el camino de coincidencia exacta añadido en el rediseño 2026-08-30 -- ver detalle completo en `docs/propuestas/guia_nuevo_matcher.md` §9.1):

| Condición | Resultado |
|---|---|
| `name_canonical` == `raw_name` exacto (case-insensitive) | `confirmed`, sin mirar nada más |
| `set_code` + tipo + idioma + `packaging` exactos, y cantidad no ambigua | `confirmed` (candidato primario, no fallback -- sin depender de `similarity`) |
| categoría con un único SKU posible en TODO el catálogo, candidato primario, idioma no contradice | `confirmed` |
| `set_code` + tipo + idioma exactos, y `similarity > 0.6` | `confirmed` (camino de siempre, sin `set_code` exacto en algún lado) |
| `set_code` + tipo coinciden, `similarity` en `[0.35, 0.6)` (o idioma/`packaging` no coincide) | `needs_review` |
| `set_code` NO coincide (aunque el tipo sí), o `similarity < 0.35` sin `set_code` exacto | `unmatched` |
| tipo `LOTE_CARTAS`/`OTROS`/`PROMO_CARD`/`MYSTERY_PACK`/`DICE_ACCESSORY` | `not_applicable` (nunca entra en el pipeline -- ver `NOT_APPLICABLE_PRODUCT_TYPES`) |

`set_code`, no `main_set`: `main_set` solo se deriva para `ONE_PIECE` (=`set_code`) y `DOUBLE_PACK` (tabla `DP<->OP`) -- para el resto de família (`ST`/`EB`/`PRB`/`DF`/Illustration Box/Playmat/Sleeves) siempre es `None` por diseño (antes se derivaba de un "OP\d+" literal en cualquier nombre, ruido sin uso real en el matching). `set_code` sigue siendo el campo que `_evaluate()`/`_best_candidate()` usan como identidad real de emparejamiento.

**Historial (entradas superseded por el Recognition Pipeline, se dejan como registro):**
