...
| **Idempotencia con `Idempotency-Key`** | Evitar duplicados si el cliente reintenta una petición que sí llegó a procesarse (ej. fallo de red justo después de crear la suscripción) | Aplicable sobre todo a `POST /subscriptions` — el navegador puede reintentar el registro de push sin miedo a crear dos suscripciones iguales |
