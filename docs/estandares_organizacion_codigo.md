...
**Hecho (2026-08-26):** `base_script.py` (1570 líneas) se partió en `domain.py`, `config.py`, `classify.py`, `http_client.py` y `dispatcher.py`, siguiendo exactamente la dirección de dependencias de la sección 2 -- `persistence.py` y `store_state.py` se dejaron intactos en su lógica (solo se actualizó su línea de import). Los 6 imports diferidos que existían (2 en `store_state.py`, 1 en `sitemap_poller.py`, 3 en `main()` de `base_script.py`) desaparecieron sin excepción: `store_state.py` ahora importa `persistence` a nivel de módulo (permitido explícitamente por la sección 2, "estado/caché puede depender de persistencia"), y `SCRAPER_CLASSES` se movió a `scrapers/__init__.py` para que tanto `persistence.py` como `dispatcher.py` puedan depender de él sin depender entre sí. `base_script.py` quedó como entry point puro (~140 líneas). Los 269 tests existentes se re-ejecutaron sin cambios de lógica y siguen en verde. Alembic (sección 6) se deja explícitamente para más adelante -- decisión del propietario del proyecto, todavía en desarrollo sin entorno de producción real.

**Actualización (2026-08-26):** `domain.py` y `classify.py` se movieron de `store_monitor/` a un paquete nuevo `shared/`, hermano de `store_monitor/` y `api/` -- `api/` necesitaba reutilizar `classify_product()` sin arrastrar las dependencias pesadas del scraper, y la solución puente original (`api/_store_monitor_bridge.py` con `sys.path.append()`) violaba la propia sección 2 de esta guía (acoplamiento de filesystem no declarado, en vez de dirección de dependencias explícita). `shared/` es ahora la capa "dominio" real de la que dependen ambos servicios como paquete instalado (Shared Kernel de DDD), no una carpeta hermana a la que se llega con trucos de `sys.path`.
